...
- `_data/theses.yml` contains every thesis topic, abstract, supervisor, tutor, status, and related links. The `status` field (`available`, `taken`, or `completed`) automatically places a thesis in the corresponding page section; completed entries can also include a student, completion year, and paper URL.
